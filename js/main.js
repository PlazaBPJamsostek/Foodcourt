...
(function () {
  ...
  function tenantCard(item, lang) {
    ...
    /* Tenant dengan `slug` -> kartu bisa diklik, membuka halaman detail.
       Tenant tanpa slug (stand "Tersedia/Available") -> kartu statis,
       tidak dianggap tenant aktif, sesuai instruksi. */
    if (item.slug) {
      return (
        '<a class="card card--tenant reveal" data-status="' + item.status + '" href="tenants/' + item.slug + '/">' +
          body +
          '<span class="cardCta">' + t(lang, "tenant.viewDetail") + icon("chevronRight", "cardCtaIcon") + "</span>" +
        "</a>"
      );
    }
    return '<article class="card card--static reveal" data-status="' + item.status + '">' + body + "</article>";
  }

  /* Daftar tenant ringkas di samping denah Lantai 8 (#floorTenantList).
     Dipakai sebagai alternatif yang akurat dari overlay klik di atas
     gambar denah, sesuai instruksi: kalau overlay posisi sulit dibuat
     akurat, lebih baik pakai daftar tenant di samping/bawah denah. */
  function floorTenantRow(item, lang) {
    const name = resolve(item.name, lang);
    const badge = resolve(item.badge, lang);
    const standNo = item.stand.replace("FC No.", "");
    const inner =
      '<span class="floorTenantStand">' + standNo + "</span>" +
      '<span class="floorTenantName">' + name + "</span>" +
      '<span class="badge badge--' + item.status + ' floorTenantBadge">' + badge + "</span>";
    if (item.slug) {
      return (
        '<a class="floorTenantRow is-link" href="tenants/' + item.slug + '/">' +
          inner + icon("chevronRight", "floorTenantChevron") +
        "</a>"
      );
    }
    return '<div class="floorTenantRow">' + inner + "</div>";
  }
  ...
  function renderAll(lang) {
    /* Setiap render dijaga dengan pengecekan elemen (defensive), supaya
       main.js juga aman dipakai di halaman detail tenant (tenants/<slug>/)
       yang tidak memiliki section-section khusus homepage ini. */
    const tenantGridEl = qs("#tenantGrid");
    if (tenantGridEl) tenantGridEl.innerHTML = tenants.map((item) => tenantCard(item, lang)).join("");
    const retailGridEl = qs("#retailGrid");
    if (retailGridEl) retailGridEl.innerHTML = retail.map((item) => tenantCard(item, lang)).join("");
    const facilityGridEl = qs("#facilityGrid");
    if (facilityGridEl) facilityGridEl.innerHTML = facilities.map((item) => facilityCard(item, lang)).join("");
    const newsListEl = qs("#newsList");
    if (newsListEl) newsListEl.innerHTML = news.map((item) => newsItem(item, lang)).join("");
    const heroPillsEl = qs("#heroPills");
    if (heroPillsEl) heroPillsEl.innerHTML = highlights.map((item) => pill(item, lang)).join("");
    const workPerksEl = qs("#workPerks");
    if (workPerksEl) workPerksEl.innerHTML = workPerks.map((item) => perkItem(item, lang)).join("");
    const floorLegendEl = qs("#floorLegend");
    if (floorLegendEl) floorLegendEl.innerHTML = floorLegend.map((item) => legendItem(item, lang)).join("");
    const floorTenantListEl = qs("#floorTenantList");
    if (floorTenantListEl) floorTenantListEl.innerHTML = tenants.map((item) => floorTenantRow(item, lang)).join("");
    const orderList = qs("#orderList");
    if (orderList) {
      orderList.innerHTML = t(lang, "order.items").map((line) => "<li>" + line + "</li>").join("");
    }
    /* Halaman detail tenant (js/tenant.js) mendaftarkan diri lewat
       window.renderTenantPage supaya ikut ter-render ulang saat bahasa
       diganti, tanpa main.js perlu tahu apa pun soal tenant.js. */
    if (typeof window.renderTenantPage === "function") window.renderTenantPage(lang);
    observeReveal();
  }
  ...
  function initNav() {
    ...
    if (!header || !hamb || !mobileNav || !backdrop) return;
